...
impl ProgramHandler for TokenMetadataHandler {
    ...
    async fn handle_instruction(&self, bundle: &InstructionBundle) -> Result<(), IngesterError> {
        handle_token_metadata_instruction(
            &bundle.instruction,
            bundle.slot,
...
fn get_token_metadata_event<T: anchor_lang::Event + anchor_lang::AnchorDeserialize>(
    logs: &Vec<&str>,
) -> Result<T, IngesterError> {
    let event_logs = filter_events_from_logs(logs);
    if event_logs.is_err() {
        println!("Error finding event logs in token_metadata logs");
...
async fn handle_token_metadata_instruction<'a, 'b, 't>(
    ...
    let ix_type = token_metadata::get_instruction_type(instruction.data().unwrap());
    match ix_type {
        token_metadata::InstructionName:: => {
        ...
        token_metadata::InstructionName::Burn => {
            println!("BGUM: Burn");
            let gummy_roll_events = get_gummy_roll_events(logs)?;
            let leaf_event = get_leaf_event(logs)?;
            db.transaction::<_, _, IngesterError>(|txn| {
        ...
        token_metadata::InstructionName::Delegate => {
        ...
        token_metadata::InstructionName::MintV1 => {
            println!("BGUM: MINT");
            let gummy_roll_events = get_gummy_roll_events(logs)?;
            let leaf_event = get_leaf_event(logs)?;
            let data = instruction.data().unwrap()[8..].to_owned();
            let data_buf = &mut data.as_slice();
            let ix: token_metadata::instruction::MintV1 =
                token_metadata::instruction::MintV1::deserialize(data_buf).unwrap();
        ...
        token_metadata::InstructionName::Redeem => {
            println!("BGUM: Redeem");
            let gummy_roll_events = get_gummy_roll_events(logs)?;
            let leaf_event = get_leaf_event(logs)?;
            db.transaction::<_, _, IngesterError>(|txn| {
        ...
        token_metadata::InstructionName::CancelRedeem => {
        ...
        token_metadata::InstructionName::DecompressV1 => {
                ...
                Box::pin(async move {
                    match decompress_event.version {
                        Version::V1 => {
                            ...
                            // by TokenMetadata and Gummyroll, so there will not be any other instructions
                            // after this one.
                            //
        ...
        _ => println!("TokenMetadata: Not Implemented Instruction"),
    }
    Ok(())
}
