...
use digital_asset_types::dao::sea_orm_active_enums::{SpecificationAssetClass, V1AccountAttachments};
...
pub async fn save_v2_master_edition(
    ...
    txn: &DatabaseTransaction) -> Result<(), IngesterError> {
    save_master_edition(V1AccountAttachments::MasterEditionV2, id, slot, me_data, txn).await
}
...
pub async fn save_v1_master_edition(
    ...
    save_master_edition(V1AccountAttachments::MasterEditionV1, id, slot,&bridge , txn).await
}
...
pub async fn save_master_edition(
    ...
    let master_edition: Option<(asset_v1_account_attachments::Model, Option<asset::Model>)> = asset_v1_account_attachments::Entity::find_by_id(id.0.to_vec())
        .find_also_related(asset::Entity)
        .join(JoinType::InnerJoin, asset::Relation::AssetData.def())
        .one(txn)
        .await?;
    let ser = serde_json::to_value(me_data).map_err(|e| {
