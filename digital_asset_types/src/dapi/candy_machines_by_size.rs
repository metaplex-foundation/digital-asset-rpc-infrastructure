...
pub async fn get_candy_machines_by_size(
    ...
    let build_candy_machine_list = filter_candy_machines?.into_iter().map(
        |(candy_machine, candy_machine_data)| async move {
            ...
            let data_end_settings = if let (Some(end_setting_number), Some(end_setting_type)) = (
            ...
            let data_config_line_settings = if let (
                Some(is_sequential),
                Some(name_length),
                Some(prefix_name),
                Some(prefix_uri),
            ...
            // TODO put all these getters in a sep file
            Ok(RpcCandyMachine {
