...
struct Paginate<E: EntityTrait + ColumnAccess, P: DbConn, R: PubkeyResult> {
    pool: Option<DbPool<P>>,
...
impl<E: EntityTrait + ColumnAccess, P: DbConn, R: PubkeyResult> Paginate<E, P, R> {
    ...
    fn pool(mut self, pool: DbPool<P>) -> Self {
        self.pool = Some(pool);
        self
    }
...
#[derive(Clone)]
struct TokenAccountPurge<P: DbConn> {
    pool: DbPool<P>,
}

impl<P: DbConn> TokenAccountPurge<P> {
    pub fn new(pool: DbPool<P>) -> Self {
        Self { pool }
    }
}

impl<P: DbConn> Purge for TokenAccountPurge<P> {
    async fn purge(&self, keys: Vec<Pubkey>) {
        let keys = keys
            .iter()
...
#[derive(Clone)]
struct MintPurge<P: DbConn> {
    pool: DbPool<P>,
}

impl<P: DbConn> MintPurge<P> {
    pub fn new(pool: DbPool<P>) -> Self {
        Self { pool }
    }
}

impl<P: DbConn> Purge for MintPurge<P> {
    async fn purge(&self, keys: Vec<Pubkey>) {
        let keys = keys
            .iter()
            .map(|k| k.to_bytes().to_vec())
...
pub async fn start_ta_purge<P: DbConn>(args: Args, db_pool: DbPool<P>, rpc: Rpc) -> Result<()> {
    ...
    let purge_handle = tokio::spawn(async move {
        let mut tasks = JoinSet::new();
        let purge = TokenAccountPurge::new(db_pool);
...
pub async fn start_mint_purge<P: DbConn>(args: Args, db_pool: DbPool<P>, rpc: Rpc) -> Result<()> {
    ...
    let purge_handle = tokio::spawn(async move {
        let mut tasks = JoinSet::new();
        let purge = MintPurge::new(db_pool);
