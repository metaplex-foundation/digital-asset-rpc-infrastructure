...
pub async fn run_inherited_sfbp_scenario_test(
    ...
    for events in seed_permutations {
        ...
        assert!(!royalty.primary_sale_happened);
...
/// Exercises batch hydrate via `get_related_for_assets` (list endpoints).
#[tokio::test]
#[serial]
#[named]
async fn test_mint_v2_inherited_sfbp_resolves_via_get_assets_by_owner() {
    let name = trim_test_name(function_name!());
    let setup = TestSetup::new_with_options(
        name.clone(),
        TestSetupOptions {
    ...
    let collection = "HgLnYtaZ9Nd1PqoGXh9rPXSMo6xUpUd6D5tsV8RUs7Qf";
    let collection_creator = "CJkzXwVwqiaSvMuRb3obrZHdrPFjCMBJBDrjspn72tDv";
    let owner = "DAS7Wnf86QNmwKWacTe8KShU7V6iw7wwcPjG9qXLPkEU";
    let tree_sig =
        "5aFu7ARYYaLXRLQAQXMfpxK26oeqA3LzuxjPJ7rXNdsXn9kySgLSQYxvpPHxje8n4yGrWjMw4Z4jjyr39W2NyEB7";
    let mint_sig =
        "4CS8k5FDLm8p1EYWyhF16nZXdG4dFmDjUah3SzSxU4QLpbthivNT26dwiAzPoXJPi3jEmXobciDpd1G71yTWci4F";
    ...
    let asset = response
        ...
        .expect("inherited SFBP asset should appear in getAssetsByOwner");

    let royalty = asset.royalty.as_ref().expect("royalty should be present");
    ...
    let creators = asset.creators.as_ref().expect("creators should be present");
    assert_eq!(creators.len(), 1);
    assert_eq!(creators[0].address, collection_creator);
    assert_eq!(creators[0].share, 100);

    let creators_raw = asset
        .creators_raw
        .as_ref()
        .expect("creators_raw should be present");
    assert!(creators_raw.is_empty());
