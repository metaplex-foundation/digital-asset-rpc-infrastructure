...
#[serial]
#[named]
async fn test_get_asset_with_show_inscription() {
    let name = trim_test_name(function_name!());
    let setup = TestSetup::new_with_options(
        name.clone(),
        TestSetupOptions {
...
#[serial]
#[named]
async fn test_show_unverified_collections_set_to_true() {
    let name = trim_test_name(function_name!());
    ...
    let seeds: Vec<SeedEvent> = seed_accounts([
        "GAinsU7ajZa3m8M25rVT1hMgRWSh7NMSjUYGNC7UVRWU",
        "BMRVjv1deZ5WvSsk5sWQfbrh2LdRS8VK2nrsVbbEao4j",
    ]);

    apply_migrations_and_delete_data(setup.db.clone()).await;
    index_seed_events(&setup, seeds.iter().collect::<Vec<&SeedEvent>>()).await;

    let request = r#"        
    {
        "id":"GAinsU7ajZa3m8M25rVT1hMgRWSh7NMSjUYGNC7UVRWU",
        "displayOptions": {
...
#[serial]
#[named]
async fn test_show_unverified_collections_set_to_false() {
    let name = trim_test_name(function_name!());
